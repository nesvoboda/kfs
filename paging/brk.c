...
{
    if (end_address < KHEAP_START)
        return -1;     
    if (end_address < KHEAP_START + kheap.size)
        return -1;     
    header_t *new_hole_address = KHEAP_START + kheap.size;

    for (int i = new_hole_address; i < end_address; i += 0x1000)
        _alloc_frame(&current_manager, i, 0, 1);

    new_hole_address->size =  usable_size(end_address - (kheap.data + kheap.size));
    footer_t *new_footer = new_hole_address + new_hole_address->size + sizeof(header_t);
